...
def gini(prob_tuple):
    sum_probs = prob_tuple[0]**2 + prob_tuple[1]**2
    gini = 1 - sum_probs
    return gini
...
def gig(starting_table, split_column, target_column):
    ...
    starting_probs = probabilities(starting_counts)
    
    #compute the gini impurity for the 3 tables
    starting_gini = gini(starting_probs)
    true_gini = gini(true_probs)
    false_gini = gini(false_probs)
